import type { Diagnosis, EncounterDiagnosisPayload, ObsPayload, VisitNotePayload } from '../types';
import {
  buildTipoDxObs,
  getCertaintyForTipo,
...
export interface ExistingEncounterDiagnosis {
  uuid?: string;
  diagnosis?: { coded?: ResourceReference };
  voided?: boolean;
}
...
export interface ObservationAlias {
  conceptUuid: string;
  formFieldPath?: string;
}

export function getReferenceUuid(reference: ResourceReference): string | undefined {
...
function matchesOwnedObservation(
  observation: ExistingEncounterObs,
  conceptUuid: string,
  formFieldPath?: string,
): boolean {
  if (observation.voided || observation.concept?.uuid !== conceptUuid) {
    return false;
  }

  if (formFieldPath) {
    return (
      observation.formFieldNamespace === TIPO_DX_FORM_FIELD_NAMESPACE && observation.formFieldPath === formFieldPath
    );
  }
...
export function findActiveObservation(
  ...
  return encounterObs.find((observation) => matchesOwnedObservation(observation, conceptUuid, formFieldPath));
}

/**
 * Reconciles one canonical observation and any explicitly known legacy aliases.
 * Foreign form namespaces are never modified. A retained value migrates to the
 * canonical concept/path; clearing voids both canonical and known legacy copies.
 */
export function reconcileObservation(
  ...
  value?: string,
  formFieldPath?: string,
  aliases: Array<ObservationAlias> = [],
): Array<ObsPayload> {
  const canonicalMatches = encounterObs.filter((observation) =>
    matchesOwnedObservation(observation, conceptUuid, formFieldPath),
  );
  const legacyMatches = aliases.flatMap((alias) =>
    encounterObs.filter((observation) => matchesOwnedObservation(observation, alias.conceptUuid, alias.formFieldPath)),
  );
  const trimmedValue = value?.trim();

  if (!trimmedValue) {
    return [...canonicalMatches, ...legacyMatches].flatMap((observation) =>
      observation.uuid ? [{ uuid: observation.uuid, voided: true as const }] : [],
    );
  }

  const existingObservation = canonicalMatches[0];
  ...
  const duplicateVoids = [...canonicalMatches.slice(1), ...legacyMatches].flatMap((observation) =>
    observation.uuid ? [{ uuid: observation.uuid, voided: true as const }] : [],
  );
...
export function reconcileEncounterDiagnoses(
  ...
  const activeExistingDiagnoses = existingDiagnoses.filter((diagnosis) => !diagnosis.voided);
  ...
  for (const selectedDiagnosis of selectedDiagnoses) {
    ...
    const existingIndex = activeExistingDiagnoses.findIndex(
      (diagnosis, index) =>
        !usedExistingIndexes.has(index) && getReferenceUuid(diagnosis.diagnosis?.coded) === codedUuid,
    );
    const existingDiagnosis = existingIndex >= 0 ? activeExistingDiagnoses[existingIndex] : undefined;
    if (existingIndex >= 0) {
...
export function reconcileDiagnosisTypeObservations(
  ...
  selectedDiagnoses.forEach((diagnosis) => {
    ...
    if (selectedPaths.has(formFieldPath)) return;
    ...
    payload.push(...reconcileObservation(encounterObs, tipoObs.concept.uuid, tipoObs.value, tipoObs.formFieldPath));
  });

...
function handleQuestion(question: FormField, page: FormPage, form: FormSchema): void {
...
function getInitialValueProperty(source: unknown, property: string): unknown {
  return isPlainObject(source) ? source[property] : undefined;
}

function getInitialObsDatetime(source: unknown): string | Date | null {
  const obsDatetime = getInitialValueProperty(source, 'obsDatetime');
  return isStringValue(obsDatetime) || isDateValue(obsDatetime) ? obsDatetime : null;
}

function getInitialComment(source: unknown): string | null {
  const comment = getInitialValueProperty(source, 'comment');
...
function handleQuestionsWithDateOptions(sectionQuestions: Array<FormField>): Array<FormField> {
  ...
  sectionQuestions?.forEach((question) => {
    ...
    if (question.type !== 'inlineDate' && isTrue(question.questionOptions?.showDate)) {
      const obsDatetime = getInitialObsDatetime(question.meta?.initialValue?.omrsObject);
...
function sanitizeQuestion(question: FormField): void {
...
function setFieldValidators(question: FormField): void {
...
function transformByType(question: FormField): void {
...
function transformByRendering(question: FormField): FormField {
...
function handleCheckbox(question: FormField): void {
...
function handleLabOrders(question: FormField): void {
...
function handleSelectConceptAnswers(question: FormField): void {
  question.questionOptions.isSearchable = true;
...
function handleProgramMetaTags(form: FormSchema): void {
...
function handleQuestionsWithObsComments(sectionQuestions: Array<FormField>): Array<FormField> {
  ...
  sectionQuestions?.forEach((question) => {
    ...
    if (question.type !== 'obsComment' && isTrue(question.questionOptions?.showComment)) {
      const comment = getInitialComment(question.meta?.initialValue?.omrsObject);
...
function handleDiagnosis(question: FormField): void {
  if (

...
import { getEffectiveRegistrationConfig, peruDiePatientIdentifierTypeUuid } from '../../../peru-registration-config';
...
const openmrsMedicalRecordIdentifierTypeUuid = '05a29f94-c0ed-11e2-94be-8c13b969e334';
...
const IdentifierInput: React.FC<IdentifierInputProps> = ({ patientIdentifier, fieldName }) => {
  ...
  const displayIdentifierName =
    patientIdentifier.identifierTypeUuid === peruDiePatientIdentifierTypeUuid
  ...
  const [hideInputField, setHideInputField] = useState(
    isSystemManagedMedicalRecord || autoGeneration || (!requiredForRegistration && initialValue === identifierValue),
  );
    ...
    */
    ...
    if (initialValue) {
      const confirmDeleteIdentifierModal = showModal('delete-identifier-confirmation-modal', {
        ...
        identifierName: displayIdentifierName,
        initialValue,
      });
    } else {
      setFieldValue('identifiers', deleteIdentifierType(values.identifiers, fieldName, identifierTypes));
  ...
  return (
    <div className={styles.IDInput}>
      {!hideInputField ? (
        <Input
          id={name}
          labelText={displayIdentifierName}
          name={name}
          disabled={disabled}
        ...
        <div className={styles.textID}>
          ...
          >
            {requiredForRegistration
              ...
              : `${t('optionalIdentifierLabel', { identifierName: displayIdentifierName })}`}
          </p>
          <p data-testid="identifier-placeholder" className={styles.bodyShort02}>
            {isSystemManagedMedicalRecord
              ? identifierValue || initialValue || t('autoGeneratedPlaceholderText', 'Auto-generated')
              : autoGeneration

...
- **Pacientes recientes**, inmediatamente después de **Buscar paciente**, abre `/recent-patients` de `esm-patient-search-app`. Exige los permisos de búsqueda e historia clínica, y respeta `search.showRecentlySearchedPatients` del buscador. Si su configuración aún no está disponible, los demás accesos siguen visibles. La lista y sus reglas de sesión permanecen en el módulo de búsqueda; Inicio no lee ni almacena pacientes.

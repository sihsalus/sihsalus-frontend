...
import { resolveMaternalForm } from './use-maternal-form-launcher';
...
describe('maternal form resolution', () => {
  ...
  it('rejects ambiguous exact matches so configuration must use a stable UUID', async () => {
    mockOpenmrsFetch.mockResolvedValueOnce({
      data: {
        results: [
          { uuid: 'first-active-form', name: 'OBST-001-ANTECEDENTES', published: true, retired: false },
          { uuid: 'second-active-form', name: 'OBST-001-ANTECEDENTES', published: true, retired: false },
        ],
      },
    } as Awaited<ReturnType<typeof openmrsFetch>>);
  ...
  it('rejects an exact match when publication metadata is missing', async () => {
    mockOpenmrsFetch.mockResolvedValueOnce({
      data: { results: [{ uuid: 'unknown-state-form', name: 'OBST-001-ANTECEDENTES' }] },
    } as Awaited<ReturnType<typeof openmrsFetch>>);

...
describe("mapeo P/D/R del diagnóstico (NTS-139)", () => {
  describe("getCertaintyForTipo", () => {
    it("Definitivo → CONFIRMED", () => {
      expect(getCertaintyForTipo(definitivoUuid, definitivoUuid)).toBe(
        "CONFIRMED",
      );
    });

    it("Presuntivo y Repetitivo → PROVISIONAL", () => {
      expect(getCertaintyForTipo(presuntivoUuid, definitivoUuid)).toBe(
        "PROVISIONAL",
      );
      expect(getCertaintyForTipo(repetitivoUuid, definitivoUuid)).toBe(
        "PROVISIONAL",
      );
    });
  });
  ...
  describe("parseTipoDxObs", () => {
    it("reconstruye el mapa { conceptUuid → tipo } e ignora obs ajenos", () => {
      const obs = [
        ...
        {
          formFieldNamespace: "other",
          formFieldPath: `${TIPO_DX_FIELD_PREFIX}x`,
          value: { uuid: "z" },
        },
        {
          formFieldNamespace: TIPO_DX_FORM_FIELD_NAMESPACE,
      ...
      expect(parseTipoDxObs(obs)).toEqual({
        [codedDiagnosisUuid]: definitivoUuid,
      });
    });
    ...
    it("ignora observaciones anuladas", () => {
      expect(
        parseTipoDxObs([
          {
            formFieldNamespace: TIPO_DX_FORM_FIELD_NAMESPACE,
    ...
    it("es inverso de buildTipoDxObs", () => {
      const built = buildTipoDxObs(
        diagnosisTypeConceptUuid,
        codedDiagnosisUuid,
        presuntivoUuid,
      );
      expect(parseTipoDxObs([built])).toEqual({
        [codedDiagnosisUuid]: presuntivoUuid,
      });
    });
  });
});

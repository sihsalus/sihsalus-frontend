...
- Al modificar medicamentos desde el historial general, el borrador conserva `urgency`, la fecha programada y la ausencia de una unidad de duración. La frecuencia codificada se conserva de forma independiente: una orden STAT diaria no se convierte en dosis única. La configuración y validación de dosis única pertenecen a `esm-patient-medications-app`.

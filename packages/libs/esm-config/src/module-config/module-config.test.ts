...
import { isUrl, validators } from '../validators/validators';
  ...
  it("doesn't mind higher-level description and validator keys", () => {
    const schema = {
      foo: {
        ...
        _validators: [validator((f) => f.bar !== f.baz, 'bar and baz must not be equal')],
        bar: {
          _default: 0,
        },
        baz: {
    ...
    const schema = {
      ...
      _validators: [
        validator(
          ...
          (v) => `The value of \`foo\` must start with the value of \`fooStart\` (which is ${v.fooStart}).`,
        ),
      ],
    };
    Config.defineConfigSchema('foo-module', schema);
...
describe('getConfig', () => {
  ...
  it('supports running validators on nested objects', async () => {
    const fooSchema = {
      bar: {
        ...
        c: { _default: 2 },
        diff: { _default: 1 },
        _validators: [validator((o) => o.a.b + o.diff === o.c, 'c must equal a.b + diff')],
      },
    };
    ...
    await Config.getConfig('foo-module');
    expect(console.error).toHaveBeenCalledWith(
      expect.stringMatching(/Invalid configuration for foo-module.bar.*c must equal a\.b \+ diff/),
    );
  ...
  it('validates config values', async () => {
    ...
    Config.provide(testConfig);
    await Config.getConfig('foo-module');
    expect(console.error).toHaveBeenCalledWith(expect.stringMatching(/bar.*foo.*must start with 'thi'.*/));
  });
  ...
  it('supports freeform object elements validations', async () => {
    ...
    Config.provide(testConfig);
    await Config.getConfig('foo-module');
    expect(console.error).toHaveBeenCalledWith(expect.stringMatching(/foo-module.foo.c.name: .*url/i));
  });
  ...
  it('supports validation of array elements', async () => {
    ...
    Config.provide(testConfig);
    await Config.getConfig('foo-module');
    expect(console.error).toHaveBeenCalledWith(expect.stringMatching(/1\.5.*foo-module.*foo\[1\].*must be an integer/));
  });
  ...
  it('supports validating structure of array element objects', async () => {
    ...
    Config.provide(testConfig);
    await Config.getConfig('foo-module');
    expect(console.error).toHaveBeenCalledWith(expect.stringMatching(/key.*foo-module.*bar\.baz\[1\]\.dingo/));
  });
  ...
  it('supports validating structure of array element nested objects', async () => {
    ...
    expect(console.error).toHaveBeenCalledWith(expect.stringMatching(/key.*array-nest.*yoshi\.nori\[1\]\.b\.shi/));
  });
  ...
  it('supports validation of nested array element objects elements', async () => {
    ...
    Config.provide(testConfig);
    await Config.getConfig('foo-module');
    expect(console.error).toHaveBeenCalledWith(expect.stringMatching(/value.*foo\[0\]\.a\.b.*must be an integer/));
  });
  ...
  it('supports validation of array element objects', async () => {
    const fooSchema = {
      bar: {
        ...
        _type: Type.Array,
        _elements: {
          _validators: [validator((o) => o.a.b + 1 === o.c, 'c must equal a.b + 1')],
        },
      },
    };
    Config.defineConfigSchema('foo-module', fooSchema);
    ...
    Config.provide(badConfig);
    await Config.getConfig('foo-module');
    expect(console.error).toHaveBeenCalledWith(expect.stringMatching(/foo-module.bar\[1\].*c must equal a\.b \+ 1/i));
...
describe('processConfig', () => {
  ...
  it('validates a config object', () => {
    ...
    expect(console.error).toHaveBeenCalledWith(expect.stringMatching(/true.*nowhere\.abe.*url/i));
...
describe('temporary config', () => {
  ...
  it('allows overriding the existing config', async () => {
    ...
    temporaryConfigStore.setState({
      config: unset(temporaryConfigStore.getState(), ['foo-module', 'foo']) as any,
    });
...
describe('extension slot config', () => {
  ...
  it('returns an object with add, remove, and order keys. No schema needs to be defined', async () => {
    ...
    const { config } = getExtensionSlotsConfigStore().getState().slots['fooSlot'];
    expect(config).toStrictEqual({
      add: ['bar', 'baz'],
      remove: ['zap'],
      order: ['qux', 'baz', 'bar'],
...
describe('extension config', () => {
  ...
  it('validates the extension configure config, with extension config schema', () => {
    ...
    Config.provide(configureConfig);
    expect(console.error).toHaveBeenCalledWith(expect.stringMatching(/unknown config key 'fooExt.no' provided.*/i));
  });
  ...
  it('does not accept module config parameters for extension if extension config schema is defined', () => {
    ...
    expect(console.error).toHaveBeenCalledWith(expect.stringMatching(/unknown config key 'fooExt.bar' provided.*/i));
  });

...
const DiagnosticoClasificado: React.FC<DiagnosticoClasificadoProps> = ({
  patientUuid,
}) => {
  ...
  const {
    ...
    pagination,
    sourceErrors,
  } = useDiagnosisHistory(patientUuid, [
      ...
      <DataTable rows={rows} headers={headers} size={isTablet ? "lg" : "sm"}>
        {({
          ...
          <TableContainer>
            <Table
              {...getTableProps()}
              aria-label={t("diagnosisHistory", "Historial de Diagnósticos")}
            >
              <TableHead>
                <TableRow>

...
const AdverseReactionFormWorkspace: React.FC<DefaultPatientWorkspaceProps> = ({
  ...
  const handleInputChange = useCallback(
    (field: keyof AdverseReaction, value: string | boolean | Date | null) => {
  ...
  const validateForm = useCallback((): boolean => {
    ...
    if (!formData.reactionDescription.trim()) {
      setError(
        t(
          "descriptionRequired",
          "Debe ingresar una descripción de la reacción",
        ),
      );
      return false;
    }
    ...
    if (!patientUuid || !locationUuid) {
      setError(
        t(
          "missingPatientOrLocation",
          "No se pudo resolver el paciente o la ubicación de sesión",
        ),
      );
      return false;
    }
    ...
    async (event: React.FormEvent) => {
      ...
      try {
        await saveAdverseReaction({
          ...
          severity: formData.severity as Exclude<
        ...
        showSnackbar({
          ...
          subtitle: t(
            "reactionSavedSubtitle",
            "La reacción adversa ha sido registrada exitosamente",
          ),
          isLowContrast: true,
        });
    ...
    <DashboardAccess privilege={credImmunizationEditPrivilege}>
      <Form className={styles.adverseReactionForm} onSubmit={handleSubmit}>
        <div style={{ padding: "1rem" }}>
          ...
          >
            <SelectItem
              text={t("selectVaccine", "Seleccione una vacuna")}
              value=""
            />
          ...
          <TextArea
            ...
            onChange={(e) =>
              handleInputChange("reactionDescription", e.target.value)
            }
            rows={4}
            placeholder={t(
              "reactionPlaceholder",
              "Describa los síntomas observados...",
            )}
            className={styles.formField}
          />
          ...
          >
            <SelectItem
              text={t("selectSeverity", "Seleccione severidad")}
              value=""
            />

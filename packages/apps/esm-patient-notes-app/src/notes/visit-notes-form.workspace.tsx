...
> = ({
    ...
    if (encounter?.diagnoses?.length) {
      try {
        const transformedDiagnoses = encounter.diagnoses
          ...
          .map((diagnosis) => ({
            patient: patientUuid,
            diagnosis: { coded: diagnosis.diagnosis.coded?.uuid },
  ...
  const onSubmit = useCallback(
    async (data: VisitNotesFormData) => {
      ...
      // persisted as valueCoded. Every field is reconciled against its existing obs:
      // retained values reuse UUIDs and cleared values explicitly void old obs.
      ...
      try {
        ...
        const response = isEditing
          ...
          : await saveVisitNote(abortController, visitNotePayload);
        ...
        try {
          ...
          // Cache revalidation is best-effort. The encounter is already committed,
          // so a consumer callback must never turn a successful clinical save into an error.
          createErrorHandler();
        }
        ...
        // Invalidate encounter and notes data since we created or updated an encounter.
        // Also invalidate visit history since the visit's encounter contents changed.
        ...
        <div className={styles.formContainer}>
          <Stack gap={2}>
                    ...
                    <div className={styles.tipoSelector}>
                      <RadioButtonGroup
                        ...
                        onChange={(value) =>
                          ...
                          setDiagnosisTipos((prev) => ({
                            ...prev,
                            [diagnosis.diagnosis.coded]: String(value),
                          }))
                        }
            ...
            <VisitNoteTextAreaRow
              ...
              placeholder={t(
                "nextAppointmentPlaceholder",
                "Date or follow-up instruction",
              )}
              rows={2}
            />
            <Row className={styles.row}>
...
> = (props) => {
  ...
  if (resolution.status !== "ready") {
    const hasDuplicates = resolution.status === "ambiguous";
    return (
      <Workspace2 title={t("visitNoteWorkspaceTitle", "Visit note")}>
        <InlineNotification
          hideCloseButton

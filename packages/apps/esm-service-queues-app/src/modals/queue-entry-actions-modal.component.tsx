...
export const QueueEntryActionModal: React.FC<QueueEntryActionModalProps> = ({
  ...
  const priorityConfigs =
    (config as ConfigObject & { priorityConfigs?: PriorityConfig[] }).priorityConfigs ??
    defaultColumnConfig.priorityConfigs;
                ...
                >
                  {priorities?.map(({ uuid, display }) => (
                    <RadioButton
                      key={uuid}
                      name={display}
                      labelText={<QueuePriority priority={{ uuid, display }} priorityConfigs={priorityConfigs} />}
                      value={uuid}
                    />
                  ))}
                </RadioButtonGroup>

...
describe('saveContact', () => {
  ...
  it('marks newly created contacts with the OCL yes concept UUID', async () => {
    ...
    const createPersonPayload = JSON.parse(createPersonOptions.body as string);

    expect(createPersonPayload.attributes).toEqual(
      expect.arrayContaining([
        {
    ...
    expect(mockOpenmrsFetch.mock.calls.map(([url]) => url)).not.toContain('/ws/rest/v1/patient');
  });
});

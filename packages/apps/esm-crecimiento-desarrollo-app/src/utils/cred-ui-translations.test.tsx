...
vi.mock(
  "../well-child-care/workspace/adverse-reaction/adverse-reaction.resource",
  () => ({
...
async function renderInSharedSlot(
...
describe.each(["en", "es"] as const)(
  "CRED UI in a shared slot (%s)",
  (language) => {
    const messages = language === "en" ? en : es;

    it("translates control status using the CRED catalog", async () => {
      await renderInSharedSlot(
        <CredTile controlNumber={1} label="1" status="overdue" />,
        language,
      );
    ...
    it("translates missing values and the available action consistently", async () => {
      await renderInSharedSlot(
        <AnemiaScreening patientUuid="synthetic-child" />,
        language,
      );
    ...
    it("uses the shared empty state without duplicating the absence message", async () => {
      ...
      expect(
        screen.getByText(
          chart.emptyStateText.replace(
            "{{displayText}}",
            messages.adverseReactions.toLowerCase(),
          ),
        ),
      ).toBeVisible();
    ...
    it("shows a readable placeholder instead of translation keys", async () => {
      ...
      expect(
        screen.getByRole("heading", {
          name:
            language === "en" ? "Section unavailable" : "Sección no disponible",
        }),
      ).toBeVisible();
      expect(
        screen.queryByText("slotComingSoonDescription"),

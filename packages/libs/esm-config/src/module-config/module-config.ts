...
import { cloneDeep, isEqual, omit as lodashOmit, mergeWith } from 'lodash-es';
import { type TemporaryConfigStore } from '..';
import type { Config, ConfigObject, ConfigSchema, ExtensionSlotConfig } from '../types';
import { Type } from '../types';
import { isArray, isBoolean, isNumber, isObject, isString, isUuid } from '../validators/type-validators';
...
function computeModuleConfig(state: ConfigInternalStore, tempState: TemporaryConfigStore) {
  for (const moduleName of Object.keys(state.schemas)) {
...
function getImplementerToolsConfig(
  ...
  for (const [config, source] of configsAndSources) {
...
function validateAllExtensionSlotConfigs(slotConfigPerModule: Record<string, Record<string, ExtensionSlotConfig>>) {
  for (const [moduleName, configBySlotName] of Object.entries(slotConfigPerModule)) {
...
function validateConfigSchema(moduleName: string, schema: ConfigSchema, keyPath = '') {
  ...
  for (const key of Object.keys(schema).filter((k) => !k.startsWith('_'))) {
    ...
    if (!Object.hasOwn(schemaPart, '_default')) {
...
function validateBranchStructure(schemaPart: ConfigSchema, value: any, keyPath: string) {
  ...
  if (isOrdinaryObject(value)) {
    ...
    } else if (!(Object.hasOwn(schemaPart, '_default') || Object.hasOwn(schemaPart, '_type'))) {
...
const setDefaults = (schema: ConfigSchema, inputConfig: Config): Config => {
  ...
  for (const key of Object.keys(schema)) {
    ...
    if (schemaPart && (Object.hasOwn(schemaPart, '_type') || Object.hasOwn(schemaPart, '_default'))) {
...
export function resetConfigSystem() {
  configSubscriptions.forEach((unsubscribe) => {
    unsubscribe();
  });

...
const mockRequirePrivilege = vi.hoisted(() =>
  vi.fn((_props: RequirePrivilegeProps): ReactNode => null),
);
...
vi.mock("@sihsalus/esm-rbac", async () => {
  ...
  return {
    ...
    RequirePrivilege: (props: RequirePrivilegeProps) =>
      mockRequirePrivilege(props),
  };
});
...
describe("epidemiological surveillance root", () => {
  ...
  it("protects direct dashboard access with the read privilege", () => {
    ...
    expect(
      screen.getByText("Epidemiological Surveillance"),
    ).toBeInTheDocument();
  });
  ...
  it("does not render the dashboard when access is denied", () => {
    ...
    expect(
      screen.queryByText("Epidemiological Surveillance"),

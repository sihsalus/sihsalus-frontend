  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.filtersPanel}>
        <Select id="resultado-indicador" labelText="Indicador" value={filters.indicador_id} onChange={(event) => { setPage(1); setFilters((current) => ({ ...current, indicador_id: event.target.value })); }}>
        ...
        <label className={styles.fieldGroup}>
          <span>Desde</span>
          <input className={styles.nativeInput} type="date" value={filters.periodo_inicio} onChange={(event) => { setPage(1); setFilters((current) => ({ ...current, periodo_inicio: event.target.value })); }} />
        </label>
        <label className={styles.fieldGroup}>
          <span>Hasta</span>
          <input className={styles.nativeInput} type="date" value={filters.periodo_fin} onChange={(event) => { setPage(1); setFilters((current) => ({ ...current, periodo_fin: event.target.value })); }} />
        </label>
      </div>
              ...
              <TableBody>
                {data.items.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>{item.periodo_inicio} - {item.periodo_fin}</TableCell>

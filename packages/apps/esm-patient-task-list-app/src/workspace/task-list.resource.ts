...
type TaskExtension = fhir.Extension & {
  valueCode?: string;
  valueString?: string;
  ...
  if (detail.extension) {
    for (const ext of detail.extension) {
      if (ext.url === 'http://openmrs.org/fhir/StructureDefinition/activity-dueKind') {
        const taskExtension = ext as TaskExtension;
        const value = taskExtension.valueCode ?? taskExtension.valueString;
        if (value === 'this-visit') {
        ...
        const ref = (ext as TaskExtension).valueReference?.reference;
...
function extractExtensionPriority(extensions: Array<TaskExtension>): Priority | undefined {

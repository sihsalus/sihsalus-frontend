...
import { fetchConfiguredLabsets } from './orders-details-table.component';
...
const concept = (uuid: string) => ({ uuid, display: `Set ${uuid}`, setMembers: [] });

describe('patient orders configured concept loader', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('returns every configured concept when all responses are valid', async () => {
    ...
    await expect(fetchConfiguredLabsets(requests)).resolves.toEqual({
      failedUuids: [],
  ...
  it('preserves valid concepts and records the UUID whose request failed', async () => {
  ...
  it('reports all configured concepts as failed when responses are rejected or mismatched', async () => {
    mockOpenmrsFetch.mockImplementation(async (url) => {

...
test('keeps the resolved form visible but blocks saving when background verification fails', async () => {
  ...
  await user.type(screen.getByRole('textbox', { name: /additional notes/i }), 'Synthetic note');
...
test('preserves non-ambulatory Visit Notes behavior for a legacy display-only diagnosis', async () => {
  ...
  await waitFor(() => expect(mockSaveCanonicalVisitNote).toHaveBeenCalledOnce());
});
...
test('typing in the diagnosis search input triggers a search', async () => {
  ...
  const diabetesMellitusTag = screen.getByTitle(/^E14\.9 - Diabetes Mellitus$/i);
...
test('renders a success snackbar upon successfully recording a visit note', async () => {
  ...
  const searchBox = screen.getByPlaceholderText('Choose a primary diagnosis');
  await user.type(searchBox, 'Diabetes Mellitus');
  const [targetSearchResult] = await screen.findAllByRole('button', {
    name: /Diabetes Mellitus/i,
  });
  expect(targetSearchResult).toBeInTheDocument();

  await user.click(targetSearchResult);

  expect(screen.getByText('Cough and fever from outpatient care')).toBeInTheDocument();
  expect(screen.queryByRole('textbox', { name: /Chief complaint/i })).not.toBeInTheDocument();
  ...
  await user.type(therapeuticIndications, 'Dieta baja en sal y caminata diaria');
  ...
  expect(
    submittedPayload.obs.some(
      (observation) =>
        ...
        observation.concept?.uuid === defaultVisitNoteClinicalConceptUuids.chiefComplaintConceptUuid,
    ),
  ).toBe(false);
  await waitFor(() =>

...
const formatDateInputValue = (date: unknown) => {
...
beforeEach(() => {
  ...
  mockOpenmrsDatePicker.mockImplementation(
    ({ id, labelText, value, onChange, invalid, invalidText, minDate, maxDate }: MockDatePickerProps) => (
      <>
        <label htmlFor={id}>{labelText}</label>
        <input
          id={id}
          type="text"
          value={value ? formatDateInputValue(value) : ''}
          data-min-date={minDate ? formatDateInputValue(minDate) : ''}
          data-max-date={maxDate ? formatDateInputValue(maxDate) : ''}
          onChange={(evt: React.ChangeEvent<HTMLInputElement>) => onChange?.(parseDateInputValue(evt.target.value))}
        />

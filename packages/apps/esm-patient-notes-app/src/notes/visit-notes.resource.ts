...
interface CanonicalVisitNoteEncounterResponse {
...
export function useVisitNotes(patientUuid: string): UseVisitNotes {
  ...
  const encountersApiUrl = patientUuid
    ? `${restBaseUrl}/encounter?${new URLSearchParams({
        patient: patientUuid,
...
function getCanonicalVisitNoteUrl(
  patientUuid: string,
  visitUuid: string,
  ...
  return `${restBaseUrl}/encounter?${new URLSearchParams({
    patient: patientUuid,
    visit: visitUuid,
    encounterType: encounterTypeUuid,
    form: formUuid,
...
/**
 * Narrows the create race after the workspace's initial 0-match resolution.
 * A backend uniqueness constraint remains the only complete cross-device
 * guarantee, but this second check prevents creating from a stale open form.
 */
export async function assertCanonicalVisitNoteCanBeCreated(
  ...
  const { data } = await openmrsFetch<CanonicalVisitNoteEncounterResponse>(
    getCanonicalVisitNoteUrl(
      patientUuid,
      visitUuid,
      encounterTypeUuid,
      formUuid,
...
/**
 * Resolves the one canonical Visit Note encounter for an active visit.
 *
 * This check lives in the workspace package rather than individual launchers so
 * every entry point gets identical create/edit semantics. A malformed response,
 * an identity mismatch, or more than one matching encounter fails closed.
 */
export function useCanonicalVisitNoteEncounter(
  ...
  const { data, error, isLoading, mutate } = useSWR<
    { data: CanonicalVisitNoteEncounterResponse },
    Error
  >(key, openmrsFetch);
  ...
  // Keep verified stale data mounted during a background revalidation. Replacing
  // the form with a loader after save would interrupt attachment handling and
  // workspace closure.
  if (isLoading || !data) {
    return {
  ...
  if (error || !Array.isArray(data.data?.results)) {
    return { status: "error", encounter: null, mutate: mutateResolution };
  }

  const encounters = data.data.results;
  const hasNextPage =
    data.data.links?.some(({ rel }) => rel === "next") ?? false;
  if (encounters.length > 1 || hasNextPage) {
    return { status: "ambiguous", encounter: null, mutate: mutateResolution };
  }

  const encounter = encounters[0];
  if (!encounter) {
    return { status: "ready", encounter: null, mutate: mutateResolution };
  }

  const encounterWithClinicalData = encounter as Encounter & {
    diagnoses?: Array<{
      diagnosis?: { coded?: unknown };
      uuid?: string;
    }>;
  ...
  const hasCompleteProviders = Boolean(
    Array.isArray(encounterWithClinicalData.encounterProviders) &&
    encounterWithClinicalData.encounterProviders.length > 0 &&
    encounterWithClinicalData.encounterProviders.every(
      (encounterProvider) =>
        encounterProvider.uuid &&
        getResourceUuid(encounterProvider.encounterRole) &&
        getResourceUuid(encounterProvider.provider),
    ),
  );
  const hasCompleteObservations = Boolean(
    Array.isArray(encounterWithClinicalData.obs) &&
    encounterWithClinicalData.obs.every(
      (obs) => obs.uuid && getResourceUuid(obs.concept),
    ),
  );
  const hasCompleteDiagnoses = Boolean(
    Array.isArray(encounterWithClinicalData.diagnoses) &&
    encounterWithClinicalData.diagnoses.every(
      (diagnosis) =>
  ...
  const isVerified = Boolean(
    ...
    getResourceUuid(encounter.visit) === visitUuid &&
    getResourceUuid(encounter.encounterType) === encounterTypeUuid &&
    getResourceUuid(encounter.form) === formUuid &&
    getResourceUuid(encounterWithClinicalData.location) &&
    hasCompleteProviders &&
    hasCompleteObservations &&
    hasCompleteDiagnoses,
  );

  return isVerified
    ? { status: "ready", encounter, mutate: mutateResolution }
...
export function fetchPrestacionalConceptsByName(
  ...
  return openmrsFetch<Array<Concept>>(url).then(({ data }) => {
    ...
    return (matchingConceptSet?.setMembers ?? [])
      .filter((concept) =>
        normalizeSearchText(concept.display).includes(normalizedSearchTerm),
      )
  ...
  for (const encounter of encounters) {
    const obs = encounter?.obs?.find(
      (observation) =>
        ...
        (!formFieldPath || observation?.formFieldPath === formFieldPath),
    );

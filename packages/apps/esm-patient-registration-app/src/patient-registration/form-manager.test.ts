...
import { createPatientWithReconciliation, FormManager, SavePatientTransactionManager } from './form-manager';
import {
...
import { type FormValues, type Patient } from './patient-registration.types';
import {
  addressUbigeoField,
...
beforeEach(() => {
  ...
  mockReconcilePatientCreation.mockResolvedValue(null);
...
describe('safe patient creation', () => {
  const scope = {
  ...
  const identifiers: Array<PatientCreationIdentifierCheckpoint> = [
    {
      fieldName: 'openmrsId',
      identifier: '10001V',
      identifierTypeUuid: 'identifier-type',
      locationUuid: 'location-uuid',
      preferred: true,
      autoGenerated: true,
  ...
  it('reconciles a lost create response without issuing a second POST', async () => {
    mockReconcilePatientCreation
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce({ data: { uuid: 'server-patient-uuid' }, ok: true } as never);
    mockSavePatient.mockRejectedValueOnce({ response: { status: 500 } });

    await expect(createPatientWithReconciliation(patient, identifiers, scope)).resolves.toMatchObject({
      data: { uuid: 'server-patient-uuid' },
    });

    expect(mockSavePatient).toHaveBeenCalledTimes(1);
    expect(loadPatientCreationCheckpoint()).toMatchObject({
      state: 'patient-confirmed',
      patientUuid: 'server-patient-uuid',
    });
  });

  it('reconciles a successful no-content response instead of repeating the POST', async () => {
    mockReconcilePatientCreation
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce({ data: { uuid: 'server-patient-uuid' }, ok: true } as never);
    mockSavePatient.mockResolvedValueOnce({ data: null, ok: true, status: 204 } as never);
  ...
  it('shares one in-flight create operation between concurrent submissions', async () => {
    let resolvePreflight!: (value: null) => void;
    mockReconcilePatientCreation.mockImplementationOnce(
      () => new Promise((resolve) => {
        resolvePreflight = resolve;
      }),
    );
    mockSavePatient.mockResolvedValueOnce({ data: { uuid: 'server-patient-uuid' }, ok: true, status: 201 } as never);

    const first = createPatientWithReconciliation(patient, identifiers, scope);
    const second = createPatientWithReconciliation(
      { ...patient, uuid: 'other-client-uuid', person: { ...patient.person, uuid: 'other-client-uuid' } },
      identifiers,
      scope,
    );
    ...
    mockReconcilePatientCreation.mockResolvedValueOnce({
      data: { uuid: 'server-patient-uuid' },
      ok: true,
    } as never);
  ...
  it('rejects a concurrent create with the same identifiers but different demographics', async () => {
    let resolvePreflight!: (value: null) => void;
    mockReconcilePatientCreation.mockImplementationOnce(
      () => new Promise((resolve) => {
        resolvePreflight = resolve;
    ...
    await expect(
      createPatientWithReconciliation(
        { ...patient, person: { ...patient.person, names: [{ ...patient.person.names[0], givenName: 'Otra' }] } },
        identifiers,
        scope,
      ),
    ).rejects.toBeInstanceOf(PatientCreationConflictError);
    ...
    resolvePreflight(null);
    await expect(first).resolves.toMatchObject({ data: { uuid: 'server-patient-uuid' } });
    expect(mockSavePatient).toHaveBeenCalledTimes(1);
  });

  it('keeps an ambiguous checkpoint and never repeats the POST', async () => {
    mockReconcilePatientCreation.mockResolvedValue(null);
    mockSavePatient.mockRejectedValueOnce({ response: { status: 500 } });

    await expect(createPatientWithReconciliation(patient, identifiers, scope)).rejects.toBeInstanceOf(
      PatientCreationAmbiguousError,
    );
    await expect(createPatientWithReconciliation(patient, identifiers, scope)).rejects.toBeInstanceOf(
      PatientCreationAmbiguousError,
    ...
    expect(mockSavePatient).toHaveBeenCalledTimes(1);
    expect(loadPatientCreationCheckpoint()).toMatchObject({ state: 'create-pending' });
  });

  it('clears the checkpoint after a definitive 4xx rejection and a valid empty reconciliation', async () => {
    mockReconcilePatientCreation.mockResolvedValue(null);
    const rejection = { response: { status: 400 } };
    mockSavePatient.mockRejectedValueOnce(rejection);

    await expect(createPatientWithReconciliation(patient, identifiers, scope)).rejects.toBe(rejection);

    expect(loadPatientCreationCheckpoint()).toBeNull();
  });

  it('keeps the checkpoint for a 499 client-closed response', async () => {
    mockReconcilePatientCreation.mockResolvedValue(null);
    mockSavePatient.mockRejectedValueOnce({ response: { status: 499 } });
  ...
  it('rejects a different user or location when a create checkpoint is pending', async () => {
    mockReconcilePatientCreation.mockResolvedValue(null);
    mockSavePatient.mockRejectedValueOnce({ response: { status: 500 } });
    await expect(createPatientWithReconciliation(patient, identifiers, scope)).rejects.toBeInstanceOf(
      PatientCreationAmbiguousError,
    ...
    await expect(
      createPatientWithReconciliation(patient, identifiers, { ...scope, userUuid: 'other-user' }),
    ).rejects.toBeInstanceOf(PatientCreationConflictError);
    expect(mockSavePatient).toHaveBeenCalledTimes(1);
  });

  it('fails closed if UUID verification and the global identifier search disagree', async () => {
    mockReconcilePatientCreation
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce({ data: { uuid: 'other-patient-uuid' }, ok: true } as never);
    mockSavePatient.mockResolvedValueOnce({ data: { uuid: 'server-patient-uuid' }, ok: true, status: 201 } as never);
  ...
  it('locks post-create retries after a relationship response is lost, including after remount', async () => {
    const session = {
      authenticated: true,
      sessionId: scope.sessionId,
      user: { uuid: scope.userUuid },
      currentProvider: { uuid: scope.providerUuid, identifier: 'PRO-1' },
    } as Session;
    const values: FormValues = {
      ...formValues,
      patientUuid: 'client-patient-uuid',
      givenName: 'Ana',
      familyName: 'Pérez',
      gender: 'F',
      birthdate: '1990-02-03',
    ...
    mockReconcilePatientCreation
      ...
      .mockResolvedValueOnce({ data: { uuid: 'server-patient-uuid' }, ok: true } as never);
    mockSavePatient.mockResolvedValueOnce({ data: { uuid: 'server-patient-uuid' }, ok: true, status: 201 } as never);
    mockSaveRelationship.mockRejectedValueOnce(new TypeError('Failed to fetch'));
    const transaction = new SavePatientTransactionManager();
    const save = (manager: SavePatientTransactionManager, submittedValues = values) =>
    ...
    await expect(save(transaction)).rejects.toBeInstanceOf(PatientRegistrationIncompleteError);
    expect(loadPatientCreationCheckpoint()).toMatchObject({
      state: 'patient-confirmed',
      patientUuid: 'server-patient-uuid',
    });
    await expect(save(transaction)).rejects.toBeInstanceOf(PatientRegistrationIncompleteError);
    await expect(
      save(new SavePatientTransactionManager(), { ...values, patientUuid: 'new-client-uuid-after-remount' }),
  ...
  it('does not POST when the checkpoint cannot be persisted', async () => {
    mockReconcilePatientCreation.mockResolvedValue(null);
  ...
  it('reuses the generated identifier from a checkpoint after a remount', async () => {
    ...
    savePatientCreationCheckpoint({
      ...
      createdAt: Date.now(),
      payloadFingerprint: 'generated-identifier-only',
      scope: { ...scope, locationUuid: 'Nyc' },
      identifiers: [
        {
          fieldName: 'foo',
          identifier: '100CHECKPOINT',
          identifierTypeUuid: 'identifierType',
          locationUuid: 'Nyc',
          preferred: true,
          autoGenerated: true,
    ...
    ).resolves.toEqual([expect.objectContaining({ identifier: '100CHECKPOINT' })]);
...
describe('FormManager', () => {
  describe('createIdentifiers', () => {
    ...
    it('rejects active identifiers when the session location is missing', async () => {
      await expect(FormManager.savePatientIdentifiers(true, undefined, formValues.identifiers, {}, '')).rejects.toMatchObject(
        {
          responseBody: {
            error: {
              message:

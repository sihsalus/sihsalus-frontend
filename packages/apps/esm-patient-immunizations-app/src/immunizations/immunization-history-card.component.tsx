...
const getNextDoseStatus = (nextDoseDate: string): 'OVERDUE' | 'DUE_SOON' | 'SCHEDULED' => {
...
const ImmunizationHistoryCard: React.FC<ImmunizationHistoryCardProps> = ({ error, immunizations }) => {
  ...
  const sortedImmunizations = useMemo(() => {
    return (immunizations || []).map((immunization) => ({
      ...
      existingDoses: (immunization?.existingDoses || [])
        .slice()
        .sort((a, b) => new Date(a?.occurrenceDateTime).getTime() - new Date(b?.occurrenceDateTime).getTime()),
    }));
  }, [immunizations]);
  ...
  const getNextDoseStatusLabel = (status: 'OVERDUE' | 'DUE_SOON' | 'SCHEDULED') => {
    if (status === 'OVERDUE') return t('overdue', 'Overdue');
    if (status === 'DUE_SOON') return t('dueSoon', 'Due soon');

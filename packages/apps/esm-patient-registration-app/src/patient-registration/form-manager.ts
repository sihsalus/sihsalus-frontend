...
import {
  ...
  type PatientCreationIdentifierCheckpoint,
  type PatientCreationCheckpointScope,
...
import {
  ...
  PatientCreationCheckpointUnavailableError,
...
function getPatientPayloadSignature(patient: Patient) {
  const { identifiers: _identifiers, uuid: _patientUuid, person } = patient;
  const { uuid: _personUuid, names, addresses, attributes, ...personDetails } = person;
  return JSON.stringify({
    person: {
      ...personDetails,
      names: names.map(({ uuid: _uuid, ...name }) => name),
      addresses: addresses.map(({ uuid: _uuid, ...address }) => address),
      attributes: attributes.map(({ uuid: _uuid, ...attribute }) => attribute),
    },
  });
}

function getPatientCreationScope(currentUser: Session, locationUuid: string): PatientCreationCheckpointScope {
  const sessionId = currentUser?.sessionId?.trim();
  const userUuid = currentUser?.user?.uuid?.trim();
  if (!sessionId || !userUuid || !locationUuid?.trim()) {
    throw new PatientCreationCheckpointUnavailableError();
  }
  return {
    sessionId,
    userUuid,
    locationUuid: locationUuid.trim(),
    providerUuid: currentUser.currentProvider?.uuid?.trim() ?? '',
  };
}

function getPatientCreationIdentifiers(
  values: FormValues,
  identifiers: Array<PatientIdentifier>,
  locationUuid: string,
): Array<PatientCreationIdentifierCheckpoint> {
  const activeEntries = Object.entries(values.identifiers ?? {}).filter(
    ([, { identifierValue, autoGeneration, selectedSource }]) =>
      Boolean(identifierValue || (autoGeneration && selectedSource)),
  );

  return activeEntries.map(([fieldName, formIdentifier], index) => {
    const identifier = identifiers[index];
    const autoGenerated = Boolean(
      formIdentifier.autoGeneration &&
        formIdentifier.selectedSource &&
        !(formIdentifier.selectedSource.autoGenerationOption?.manualEntryEnabled && formIdentifier.identifierValue),
    );
    if (!identifier?.identifier?.trim() || !formIdentifier.identifierTypeUuid?.trim() || !locationUuid?.trim()) {
      throw new PatientCreationConflictError();
    }
    return {
      fieldName,
      identifier: identifier.identifier.trim(),
      identifierTypeUuid: formIdentifier.identifierTypeUuid,
      locationUuid,
      preferred: Boolean(formIdentifier.preferred),
      autoGenerated,
      ...(autoGenerated ? { sourceUuid: formIdentifier.selectedSource.uuid } : {}),
    };
  });
}

let activePatientCreation:
  | {
      identifiers: Array<PatientCreationIdentifierCheckpoint>;
      payloadSignature: string;
      scope: PatientCreationCheckpointScope;
      operation: Promise<FetchResponse>;
    }
  | undefined;

export function createPatientWithReconciliation(
  patient: Patient,
  identifiers: Array<PatientCreationIdentifierCheckpoint>,
  scope: PatientCreationCheckpointScope,
  signal?: AbortSignal,
): Promise<FetchResponse> {
  const payloadSignature = getPatientPayloadSignature(patient);
  if (activePatientCreation) {
    return checkpointsHaveSameIdentifiers(activePatientCreation.identifiers, identifiers) &&
      activePatientCreation.payloadSignature === payloadSignature &&
      checkpointsHaveSameScope(activePatientCreation.scope, scope)
      ? activePatientCreation.operation
      : Promise.reject(new PatientCreationConflictError());
  }

  const operation = runPatientCreationWithReconciliation(patient, identifiers, payloadSignature, scope, signal).finally(() => {
    if (activePatientCreation?.operation === operation) {
      activePatientCreation = undefined;
    }
  });
  activePatientCreation = { identifiers, payloadSignature, scope, operation };
  return operation;
}

async function runPatientCreationWithReconciliation(
  patient: Patient,
  identifiers: Array<PatientCreationIdentifierCheckpoint>,
  payloadSignature: string,
  scope: PatientCreationCheckpointScope,
  ...
  const payloadFingerprint = await fingerprintPatientPayload(payloadSignature);
  if (!payloadFingerprint) {
    throw new PatientCreationCheckpointUnavailableError();
  }
  const pendingCheckpoint = loadPatientCreationCheckpoint();
  if (pendingCheckpoint) {
    if (
      !checkpointsHaveSameIdentifiers(pendingCheckpoint.identifiers, identifiers) ||
      pendingCheckpoint.payloadFingerprint !== payloadFingerprint ||
      !checkpointsHaveSameScope(pendingCheckpoint.scope, scope)
    ) {
      throw new PatientCreationConflictError();
    }
    try {
      let reconciled: FetchResponse | null = null;
      if (pendingCheckpoint.patientUuid) {
        try {
          reconciled = (await fetchAndVerifyCreatedPatient(
            pendingCheckpoint.patientUuid,
            identifiers,
            patient,
          )) as FetchResponse;
        } catch (error) {
          if (error instanceof PatientCreationConflictError) {
            throw error;
          }
        }
      }
      reconciled ??= (await reconcilePatientCreation(identifiers, patient)) as FetchResponse | null;
      if (!reconciled) {
        throw new PatientCreationAmbiguousError();
      }
      return Object.assign(reconciled as FetchResponse, { patientCreationRecoveredFromCheckpoint: true });
    } catch (error) {
      if (error instanceof PatientCreationConflictError || error instanceof PatientCreationAmbiguousError) {
        throw error;
      }
      throw new PatientCreationAmbiguousError();
    }
  ...
  const existingPatient = await reconcilePatientCreation(identifiers, patient);
  if (existingPatient) {
    throw new PatientCreationConflictError();
  }
  ...
  try {
    response = await savePatient(patient, undefined, signal);
  } catch (error) {
    try {
      const reconciled = await reconcilePatientCreation(identifiers, patient);
      if (reconciled) {
        const checkpointSaved = savePatientCreationCheckpoint({
          version: 2,
          state: 'patient-confirmed',
          createdAt: Date.now(),
          identifiers,
          payloadFingerprint,
          scope,
          patientUuid: reconciled.data.uuid,
        });
        if (!checkpointSaved) {
          throw new PatientCreationAmbiguousError();
        }
        return reconciled as FetchResponse;
      }
    } catch (reconciliationError) {
      if (reconciliationError instanceof PatientCreationConflictError) {
    ...
    if (isDefinitivePatientCreateRejection(error)) {
      if (!clearPatientCreationCheckpoint()) {
        throw new PatientCreationAmbiguousError();
      }
      throw error;
    }
  ...
  const patientUuid = typeof response.data?.uuid === 'string' ? response.data.uuid.trim() : '';
  try {
    let verifiedByUuid: FetchResponse | null = null;
    if (patientUuid) {
      try {
        verifiedByUuid = (await fetchAndVerifyCreatedPatient(patientUuid, identifiers, patient)) as FetchResponse;
      } catch (error) {
        if (error instanceof PatientCreationConflictError) {
          throw error;
        }
    ...
    const verifiedBySearch = (await reconcilePatientCreation(identifiers, patient)) as FetchResponse | null;
    if (verifiedByUuid && verifiedBySearch && verifiedByUuid.data.uuid !== verifiedBySearch.data.uuid) {
      throw new PatientCreationConflictError();
    }
    const verified = verifiedBySearch ?? (!patientUuid ? verifiedByUuid : null);
    if (
      verified &&
      (!verifiedByUuid || !patientUuid || verifiedByUuid.data.uuid === verified.data.uuid)
    ) {
      const checkpointSaved = savePatientCreationCheckpoint({
        version: 2,
        state: 'patient-confirmed',
        ...
        payloadFingerprint,
        scope,
        patientUuid: verified.data.uuid,
      });
      if (!checkpointSaved) {
        throw new PatientCreationAmbiguousError();
      }
      return verified as FetchResponse;
    }
  } catch (error) {
    if (error instanceof PatientCreationConflictError) {
      throw error;
...
export const REGISTRATION_DOMAIN_ERROR = 'REGISTRATION_DOMAIN_ERROR';

function registrationError(message: string) {
  return {
    code: REGISTRATION_DOMAIN_ERROR,
    responseBody: {
      error: { message },
    },
  };
...
function ensureTransactionState(transactionManager: SavePatientTransactionManager) {
  ...
  transactionManager.postCreateOutcomeAmbiguous ??= false;
...
export class FormManager {
  ...
  static savePatientFormOnline: SavePatientForm = async (
    ...
    const usesPatientCreationCheckpoint = isNewPatient && !personUuidToPromote;

    if (usesPatientCreationCheckpoint && savePatientTransactionManager.postCreateOutcomeAmbiguous) {
      throw new PatientRegistrationIncompleteError(savePatientTransactionManager.savedPatientUuid);
    }

    if (personUuidToPromote && loadPatientCreationCheckpoint()) {
      throw new PatientCreationConflictError();
    }
    ...
    const patientCreationIdentifiers = usesPatientCreationCheckpoint
      ? getPatientCreationIdentifiers(values, patientIdentifiers, currentLocation)
      : [];
    const patientCreationScope = usesPatientCreationCheckpoint
      ? getPatientCreationScope(currentUser, currentLocation)
      : undefined;
    ...
    const savePatientResponse =
      ...
      savePatientTransactionManager.savedPatientUuid
        ...
        : usesPatientCreationCheckpoint
          ? await createPatientWithReconciliation(
              createdPatient,
              patientCreationIdentifiers,
              patientCreationScope as PatientCreationCheckpointScope,
              signal,
            )
          : await savePatient(
              createdPatient,
    ...
    if (savePatientResponse.ok) {
      ...
      if (
        usesPatientCreationCheckpoint &&
        (savePatientResponse as FetchResponse & { patientCreationRecoveredFromCheckpoint?: boolean })
          .patientCreationRecoveredFromCheckpoint
      ) {
        savePatientTransactionManager.postCreateOutcomeAmbiguous = true;
        throw new PatientRegistrationIncompleteError(savePatientResponse.data.uuid);
      }
      ...
      try {
        ...
        if (capturePhotoProps?.imageData && !savePatientTransactionManager.photoSaved) {
          ...
          if (patientPhotoConceptUuid) {
            try {
              ...
              console.warn('Patient photo endpoint rejected the image; attempting the attachment fallback.', error);
        ...
        if (usesPatientCreationCheckpoint && !isDefinitivePatientCreateRejection(error)) {
          savePatientTransactionManager.postCreateOutcomeAmbiguous = true;
          throw Object.assign(new PatientRegistrationIncompleteError(savePatientResponse.data.uuid), { cause: error });
        }
        throw error;
      }
    }

    if (usesPatientCreationCheckpoint) {
      if (!clearPatientCreationCheckpoint()) {
        savePatientTransactionManager.postCreateOutcomeAmbiguous = true;
        throw new PatientRegistrationIncompleteError(savePatientResponse.data.uuid);
      }
    ...
    if (documentTypeConceptUuid && documentNumber) {
      ...
      try {
        ...
        if (outcome.status === 'verified') {
          ...
          throw registrationError(
            `Los datos de la persona no coinciden con la fuente de verificación: ${outcome.observation}. Resuelva la discrepancia antes de promover.`,
          );
        }
      } catch (error) {
      ...
      */
      .map(async ([identifierFieldName, patientIdentifier]) => {
        ...
        if (autoGeneration && !autoGenerationManualEntry) {
          ...
          if (!identifier) {
            const patientCreationCheckpoint = isNewPatient ? loadPatientCreationCheckpoint() : null;
            const checkpointIdentifier = patientCreationCheckpoint?.identifiers.find(
                  (candidate) =>
                    candidate.autoGenerated &&
                    candidate.fieldName === identifierFieldName &&
                    candidate.identifierTypeUuid === identifierTypeUuid &&
                    candidate.locationUuid === location &&
                    candidate.sourceUuid === selectedSource?.uuid,
                );
            if (patientCreationCheckpoint && !checkpointIdentifier) {
              throw new PatientCreationConflictError();
            }
            identifier = checkpointIdentifier?.identifier ?? (await generateIdentifier(selectedSource.uuid, signal)).data.identifier;
...
export class SavePatientTransactionManager {
  ...
  patientSaved = false;
  photoSaved = false;
  postCreateOutcomeAmbiguous = false;

...
function buildEncounter(
  ...
  return {
    ...
    encounterDatetime: options.encounterDatetime ?? '2026-07-01T10:00:00.000+0000',
    encounterType: options.encounterTypeUuid ? { uuid: options.encounterTypeUuid } : undefined,
...
describe('useVisitNoteClinicalContext procedures fallback', () => {
  ...
  it('reads non-pharmacological instructions from their dedicated concept', async () => {
      ...
      data: {
        results: [
          buildEncounter('patient-therapeutic-indications', [
            {
              conceptUuid: defaultVisitNoteClinicalConceptUuids.therapeuticIndicationsConceptUuid,
    ...
    const { result } = renderHook(() => useVisitNoteClinicalContext('patient-therapeutic-indications'));
    await waitFor(() => expect(result.current.isLoading).toBe(false));

    expect(result.current.clinicalContext.therapeuticIndications).toBe('Dieta baja en sal y caminata diaria');
  });
  ...
  it('excludes the visit-note encounter so outpatient care remains the summary source', async () => {
      ...
      data: {
        results: [
          buildEncounter(
            ...
            [
              {
                conceptUuid: defaultVisitNoteClinicalConceptUuids.chiefComplaintConceptUuid,
    ...
    const { result } = renderHook(() => useVisitNoteClinicalContext(patientUuid, visitUuid));
    await waitFor(() => expect(result.current.isLoading).toBe(false));

    expect(result.current.clinicalContext.chiefComplaint).toBe('Original outpatient complaint');
  });
});

...
describe("service queue route privilege contract", () => {
  it("exposes the active queue count on the home metrics slot", () => {
    expect(
      routes.extensions.find(({ name }) => name === "patients-in-queue-tile"),
    ).toMatchObject({
  ...
  it("protects both active-visit queue workspaces with visit mutation privileges", () => {
    const legacyWorkspace = routes.workspaces.find(
      ({ name }) => name === "create-queue-entry-workspace",
    );
    const workspace = routes.workspaces2.find(
      ({ name }) => name === "queue-patient-search-add-to-queue-workspace",
    );

    expect(legacyWorkspace?.privileges).toEqual(
      expect.arrayContaining(visitMutationPrivileges),
    );
    expect(workspace?.privileges).toEqual(
      expect.arrayContaining(visitMutationPrivileges),
    );
  });
  ...
  it("keeps visit creation privileges on the start-visit child instead of every queue-entry branch", () => {
    ...
    const queueEntryLaunchSurfaces = [
      routes.workspaces.find(
        ({ name }) => name === "create-queue-entry-workspace",
      ),
      routes.workspaces2.find(
        ({ name }) => name === "queue-patient-search-workspace",
      ),
      queueEntryWorkspace,
    ];

    expect(startVisitWorkspace?.privileges).toEqual(
      expect.arrayContaining(["Add Visits", "Get Visit Types"]),
    );
    expect(startVisitWorkspace?.privileges).toEqual(
      expect.arrayContaining(queueEntryWorkspace?.privileges ?? []),
    );
  ...
  it("requires the edit privilege on every queue mutation surface", () => {
    const editingSurfaces = [
      ...
      ...routes.modals,
      ...routes.workspaces.filter(
        ({ name }) => name !== "service-queues-linelist-filter",
      ),
    ...
    editingSurfaces.forEach(({ privileges }) => {
      expect(Array.isArray(privileges) ? privileges : [privileges]).toContain(
  ...
  it("keeps both companion workspaces in the queue entry window", () => {
    expect(
      ...
      {
        component:
          "@sihsalus/esm-patient-chart-app#companionPersonSearchWorkspace",
        name: "queue-visit-companion-search-workspace",
        window: "add-queue-entry",
      },
      {
        component:
          "@sihsalus/esm-patient-chart-app#companionPersonRegistrationWorkspace",
  ...
  it("requires the native OpenMRS person privileges for companion workflows", () => {
    expect(
      routes.workspaces2.find(
        ({ name }) => name === "queue-visit-companion-search-workspace",
      )?.privileges,
    ).toEqual(
      expect.arrayContaining(["app:home.colasAtencion.editar", "Get People"]),
    );
    expect(
      routes.workspaces2.find(
        ({ name }) => name === "queue-visit-companion-registration-workspace",
      )?.privileges,
    ).toEqual(
  ...
  it("reserves bulk queue clearing for the dedicated capability", () => {
    const clearExtension = routes.extensions.find(
      ({ name }) => name === "clear-all-queue-entries",
    );
    const clearModal = routes.modals.find(
      ({ name }) => name === "clear-all-queue-entries-modal",
    );
  ...
  it("uses clinical privileges for clinical workspaces embedded in queues", () => {
    const vitalsWorkspace = routes.workspaces2.find(
      ({ name }) => name === "service-queues-patient-vitals-workspace",
    );
    const visitNotesWorkspace = routes.workspaces2.find(
      ({ name }) => name === "service-queues-visit-notes-workspace",
    );

    expect(vitalsWorkspace?.privileges).toContain(
      "app:hoja.clinica.signosVitales.editar",
    );
    expect(visitNotesWorkspace?.privileges).toContain(
  ...
  it("requires native catalog permissions on queue and room administration workspaces", () => {
    const queueWorkspaces = [
      ...routes.workspaces,
      ...routes.workspaces2,
    ].filter(({ name }) => name === "service-queues-service-form");

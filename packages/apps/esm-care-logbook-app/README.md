...
- Vista inicial **Hoy**, según America/Lima. **Rango de fechas** propone los últimos 30 días inclusivos y exige ambas fechas válidas antes de consultar; **Todo el histórico** es una elección explícita. No se solicita todo el histórico si se borra una fecha del rango.
- Tabla de seis columnas operativas: fecha/hora, paciente con HCE o código temporal y documento visible cuando existe (DNI, CE u otro tipo real), tipo de visita, UPSS, estado de atención y SIS. Cada fila usa la expansión de Carbon para mostrar identificación, responsable, nacimiento, edad, sexo, dirección y comunicación sin perder esos datos de la búsqueda o del CSV. La tabla mantiene desplazamiento horizontal contenido en pantallas pequeñas.
- **Limpiar filtros** restablece también el periodo a Hoy y las fechas sugeridas, además de búsqueda, tipo, UPSS y estado.
- Filtros por tipo de atención (`visitType`), UPSS y estado, además de búsqueda libre dentro del periodo consultado. Las opciones conservan el valor seleccionado al cambiar el periodo para que no quede un filtro invisible. Incluye aperturas activas y finalizadas, ordenadas de más reciente a más antigua.

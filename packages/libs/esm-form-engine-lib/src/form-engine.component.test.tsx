...
const mockFormProcessorFactory = vi.fn((_props: { onDependencyError: (error: unknown) => void }) => null);
const mockUsePatientData = vi.fn();

vi.mock('.', () => ({
  isEmpty: (value: unknown) => value === null || value === undefined || value === '',
  useFormJson: () => ({
    formError: undefined,
    formJson: {
      name: 'Synthetic form',
...
vi.mock('./components/processor-factory/form-processor-factory.component', () => ({
  default: (props: { onDependencyError: (error: unknown) => void }) => mockFormProcessorFactory(props),
}));
...
vi.mock('./provider/form-factory-provider', () => ({
  FormFactoryProvider: ({ children }: { children: React.ReactNode }) => children,
}));
...
describe('FormEngine encounter load failure', () => {
  ...
  it('replaces the editable form with a safe error state after a 403', () => {
    ...
    act(() => {
      mockFormProcessorFactory.mock.calls.at(-1)?.[0].onDependencyError(forbiddenError);
    });
    ...
    expect(consoleError).toHaveBeenCalledWith('Failed to load data required by the form.');
  });

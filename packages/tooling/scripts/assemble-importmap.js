...
function patchIndexHtml() {
  ...
  const publicSpaUrl =
    process.env.SIHSALUS_PUBLIC_SPA_URL || process.env.SIHSALUS_PUBLIC_URL || process.env.PUBLIC_URL || '';
  const backendUrl = process.env.SIHSALUS_BACKEND_URL || '';
  const { baseUrl: socialPreviewBaseUrl, source: socialPreviewSource } = resolveSocialPreviewBaseUrl({
    spaPath,
    publicSpaUrl,
    backendUrl,
  });

  if (socialPreviewSource === 'invalid-backend-url') {
    logWarn(`Ignoring malformed backend URL for social preview: ${backendUrl}`);
  }

  if (socialPreviewSource === 'invalid-backend-url' || socialPreviewSource === 'spa-path') {
    logWarn(
      'SIHSALUS_PUBLIC_SPA_URL is not set and SIHSALUS_BACKEND_URL could not be used. Falling back to relative SPA path for social tags.',
    );
  }

  if (!isAbsoluteHttpUrl(socialPreviewBaseUrl)) {
    logWarn(
      `Using non-absolute social preview URL: ${socialPreviewBaseUrl}. Set SIHSALUS_PUBLIC_SPA_URL to a public HTTPS URL for link previews.`,
    );
  }
  ...
  html = applySocialPreview(html, { baseUrl: socialPreviewBaseUrl, locale: defaultLocale });

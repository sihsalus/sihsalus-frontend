...
import type {
  CompletedFormInfo,
  Form,
} from "@sihsalus/esm-sihsalus-shared/src/ui/forms-selector/types";
...
const CREDFormsSelectorWorkspace: React.FC<CREDFormsSelectorWorkspaceProps> = (
  props,
) => {
  ...
  const patientBirthDate =
    props.patientBirthDate ?? workspaceProps.patientBirthDate;
  const controlNumber =
    props.controlNumber ?? workspaceProps.controlNumber ?? 0;
  const controlTargetDate =
    props.controlTargetDate ?? workspaceProps.controlTargetDate;
  const title =
    props.title ??
    workspaceProps.title ??
    t(
      "credFormsSelection",
      "Selección de Formularios Crecimiento y Desarrollo",
    );
  const subtitle =
    props.subtitle ??
  ...
  const availableForms =
    providedAvailableForms.length > 0
      ? providedAvailableForms
      : fallbackAvailableForms;
  const filteredAvailableForms = useMemo(
    () =>
      availableForms.filter((formInfo) =>
        userHasAccess(
          formInfo.requiredPrivilege ?? credCourseLifeEditPrivilege,
          session?.user,
        ),
      ),
  ...
  const launchForm = useCallback(
    async (form: Form, encounterUuid: string) => {
      try {
        const resolvedForm = await resolveCREDForm(
          form.uuid,
          form.display ?? form.name ?? form.uuid,
        );

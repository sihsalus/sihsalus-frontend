...
export function reconcileObservation(
  ...
  const legacyMatches = aliases.flatMap((alias) =>
    encounterObs.filter((observation) =>
      matchesOwnedObservation(
        observation,
        alias.conceptUuid,
        alias.formFieldPath,
      ),
  ...
  const duplicateVoids = [
    ...canonicalMatches.slice(1),
    ...legacyMatches,
  ].flatMap((observation) =>
  ...
  for (const selectedDiagnosis of selectedDiagnoses) {
    ...
    const existingIndex = activeExistingDiagnoses.findIndex(
      (diagnosis, index) =>
        ...
        getReferenceUuid(diagnosis.diagnosis?.coded) === codedUuid,
    );

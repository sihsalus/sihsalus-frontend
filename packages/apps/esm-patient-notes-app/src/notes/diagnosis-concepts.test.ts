...
describe('fetchDiagnosisConceptsByName', () => {
  ...
  it('finds and prioritizes an exact CIE-10 code stored without a dot', async () => {
    ...
    const diagnoses = await fetchDiagnosisConceptsByName('K71.0', 'diagnosis class');

    expect(diagnoses.map(({ uuid }) => uuid)).toEqual(['diagnosis-k710', 'diagnosis-a710']);
    expect(mockOpenmrsFetch).toHaveBeenNthCalledWith(1, expect.stringContaining('name=K710'));
    expect(mockOpenmrsFetch).toHaveBeenNthCalledWith(2, expect.stringContaining('name=K71.0'));
    expect(mockOpenmrsFetch).toHaveBeenNthCalledWith(1, expect.stringContaining('class=diagnosis%20class'));
  });
  ...
  it('requests enough source metadata to validate a display-only CIE-10 mapping', async () => {
    ...
    const [diagnosis] = await fetchDiagnosisConceptsByName('diabetes', 'diagnosis-class-uuid');

    expect(getCie10MappedCode(diagnosis)).toBe('E11.9');
    expect(mockOpenmrsFetch).toHaveBeenCalledWith(
      expect.stringContaining('conceptMappings:(conceptReferenceTerm:(conceptSource:(name,display),code))'),
    );
  });
  ...
  it('encodes a diagnosis name without treating it as a CIE-10 code', async () => {
    ...
    await fetchDiagnosisConceptsByName('dolor & fiebre', 'diagnosis-class-uuid');

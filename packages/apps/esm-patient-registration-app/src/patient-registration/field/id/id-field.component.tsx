...
export const Identifiers: React.FC = () => {
  ...
  const { defaultPatientIdentifierTypes, hiddenPatientIdentifierTypeUuids } = config;
  const hiddenIdentifierTypeUuids = hiddenPatientIdentifierTypeUuids ?? noHiddenPatientIdentifierTypeUuids;
  const hasVisibleIdentifierTypes = identifierTypes.some((type) => !hiddenIdentifierTypeUuids.includes(type.uuid));
  ...
  useEffect(() => {
    if (identifierTypes) {
      ...
      identifierTypes
        .filter((type) => !hiddenIdentifierTypeUuids.includes(type.uuid))
        .filter(
          (type) =>
            type.isPrimary ||
            type.required ||
  ...
  const identifierFieldNames = Object.keys(values.identifiers ?? {})
    .filter((fieldName) => {
      const identifierTypeUuid = values.identifiers[fieldName]?.identifierTypeUuid;
      return !identifierTypeUuid || !hiddenIdentifierTypeUuids.includes(identifierTypeUuid);
    })
    .sort(
  ...
  return (
    <div className={styles.halfWidthInDesktopView}>
      ...
      <div className={styles.identifierFieldsGrid}>
        ...
        {showIdentifierOverlay && hasVisibleIdentifierTypes ? (
          <IdentifierSelectionOverlay
            setFieldValue={setFieldValue}
            closeOverlay={closeIdentifierSelectionOverlay}
            hiddenPatientIdentifierTypeUuids={hiddenIdentifierTypeUuids}
          />
        ) : null}
      </div>
    </div>

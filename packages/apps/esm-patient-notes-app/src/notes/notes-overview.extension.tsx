...
import { AddIcon, userHasAccess, useLayoutType, useSession } from '@openmrs/esm-framework';
import {
  CardHeader,
  EmptyState,
  ErrorState,
 ...
 */
const AuthorizedNotesOverview: React.FC<NotesOverviewProps & { canEdit: boolean }> = ({
  patientUuid,
  patient,
  basePath: _basePath,
  canEdit,
...
const NotesOverview: React.FC<NotesOverviewProps> = (props) => {
  ...
  return <AuthorizedNotesOverview {...props} canEdit={userHasAccess(visitNotesEditPrivilege, session?.user)} />;
};

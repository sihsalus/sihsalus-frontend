...
import {
  fhirImmunizationConceptMappingLabels,
  getFhirImmunizationConceptMappings,
} from "./fhir-immunization-config";
...
const ImmunizationsForm: React.FC<
  PatientWorkspace2DefinitionProps<Record<string, never>, Record<string, never>>
> = ({
  closeWorkspace,
  ...
  const fhirConceptMappings = getFhirImmunizationConceptMappings(
    config?.fhirConceptMappings,
  );
  ...
  const immunizationFormSchema = useMemo(() => {
    return z
      .object({
        vaccineUuid: z
          .string()
          .min(1, t("vaccineRequired", "Vaccine is required")),
        vaccinationDate: z
          .date()
          .min(new Date(patient.birthDate), {
            message: t(
              "vaccinationDateCannotBeBeforeBirthDate",
              "Vaccination date cannot precede birth date",
            ),
          })
          .refine(
            (date) => {
            ...
            {
              message: t(
                "vaccinationDateCannotBeInTheFuture",
        ...
        doseNumber: z
          .union([z.number({ coerce: true }).int().min(1), z.null()])
          .optional(),
        ...
        programContext: z
          .enum(["routine", "catch-up", "campaign", "special"])
          .default("routine"),
        ...
        if (value.status === "not-done" && !value.statusReason?.trim()) {
          ctx.addIssue({
            ...
            message: t(
              ...
              "Please provide the reason for non-administration or deferral",
            ),
          });
        }
      });
  ...
  const duplicateDoseWarning = useMemo(() => {
    ...
    return isDuplicate
      ? t(
          "duplicateDoseWarning",
          "Dose {{dose}} has already been recorded for this vaccine",
          { dose: doseNumber },
        )
      : undefined;
  }, [
  ...
  const minsaAgeWarning = useMemo(() => {
    if (!selectedSequence || !vaccinationDate || !patient.birthDate)
      return null;
    ...
    const ageInDays = dayjs(vaccinationDate)
      .startOf("day")
      .diff(dayjs(patient.birthDate).startOf("day"), "day");
    ...
    if (typeof minAge === "number" && ageInDays < minAge) {
      return t(
        ...
        {
          label: selectedSequence.minsaLabel
            ? ` (${selectedSequence.minsaLabel})`
            : "",
        },
      );
    }
  ...
  useEffect(() => {
    const sub = immunizationFormSub.subscribe((props) => {
      if (props) {
        const vaccinationDateOrNow = props.vaccinationDate
          ? parseDate(props.vaccinationDate)
          : new Date();
        reset({
  ...
  useEffect(() => {
    ...
    const currentIndex = sequenceDefinition.sequences.findIndex(
      (sequence) => sequence.sequenceNumber === doseNumber,
    );
    const nextSequence =
      currentIndex >= 0
        ? sequenceDefinition.sequences[currentIndex + 1]
        : undefined;

    if (nextSequence?.intervalInDaysAfterPreviousDose) {
      setValue(
        "nextDoseDate",
        dayjs(vaccinationDate)
          .add(nextSequence.intervalInDaysAfterPreviousDose, "day")
          .toDate(),
        {
          shouldDirty: false,
        },
    ...
    activeScheduleSequenceDefinitions,
    doseNumber,
    setValue,
    vaccinationDate,
    vaccineUuid,
  ...
  const existingDoseNumbers = useMemo(() => {
    const immunization = existingImmunizations?.find(
      (candidate) => candidate.vaccineUuid === vaccineUuid,
    );
    ...
    async (data: ImmunizationFormInputData) => {
      try {
        ...
        const immunization: ImmunizationFormData = {
          ...
          vaccineName:
            immunizationsConceptSet.answers.find(
              (answer) => answer.uuid === vaccineUuid,
            )?.display ?? "",
          vaccineUuid: vaccineUuid,
          vaccinationDate: dayjs(vaccinationDate)
            .startOf("day")
            .toDate()
            .toISOString(),
          doseNumber,
          ...
          nextDoseDate: nextDoseDate
            ? dayjs(nextDoseDate).startOf("day").toDate().toISOString()
            : null,
          note,
        ...
        if (immunizationToEditMeta?.persistenceSource === "ampath-form") {
          ...
          try {
            ...
            const errorDetails = getImmunizationSaveErrorDetails(
              err,
              fhirConceptMappings,
            );
            ...
            : errorDetails.type === "fhir-setup"
              ...
              : errorDetails.type === "validation"
                ? (errorDetails.diagnostics ??
                  t(
                    "invalidVaccinationRequest",
                    "The vaccination request is invalid.",
                  ))
                : (errorDetails.message ??
                  t(
                    "unknownVaccinationSaveError",
                    "Unknown error while saving vaccination.",
                  ));
  ...
  return (
    <Workspace2
      title={t("immunizationWorkspaceTitle", "Immunization")}
      hasUnsavedChanges={isDirty}
    >
      {canEditImmunizations ? (
            ...
            <Stack gap={5} className={styles.container}>
                ...
                <Controller
                  ...
                  render={({ field: { onChange, value } }) => (
                    <Dropdown
                      ...
                      itemToString={(item) =>
                        immunizationsConceptSet?.answers.find(
                          (candidate) => candidate.uuid === item,
                        )?.display
                      }
                      items={
                        immunizationsConceptSet?.answers?.map(
                          (item) => item.uuid,
                        ) || []
                      }
                  ...
                  <Controller
                    ...
                    render={({ field: { onChange, value } }) => (
                      <TextInput
                        ...
                        labelText={t(
                          "statusReason",
                          "Motivo de no aplicación o diferimiento",
                        )}
              ...
              {minsaAgeWarning ? (
                <InlineNotification
                  ...
                  lowContrast
                  title={t(
                    "minsaScheduleWarning",
                    "Advertencia de esquema MINSA",
                  )}
                  ...
                  render={({ field: { onChange, value } }) => (
                    <TextArea
                      ...
                      placeholder={t(
                        ...
                        "For example: mild redness at injection site",
                      )}
                      value={value}
                    />
                  )}

...
describe("useConsultaExternaFormLauncher", () => {
  ...
  it("creates the sole-per-visit form when no matching encounter exists and attaches the verified visit", async () => {
    ...
    expect(encounterQuery.searchParams.get("form")).toBe(formUuid);
    expect(encounterQuery.searchParams.get("limit")).toBe("2");
    ...
    const workspaceProps = mockLaunchPatientWorkspace.mock.calls[0][1] as {
      mutateForm: () => void;
    };

  ...
  mockFetch
    ...
    .mockResolvedValueOnce({
      data: {
        results: [
          exactEncounter("two"),
          { ...exactEncounter("wrong-form"), form: { uuid: "other-form" } },
        ],
      },
    } as never);
  const result = await (
    fetcher as () => Promise<{ data: { results: Array<{ uuid: string }> } }>
  )();
  expect(result.data.results.map(({ uuid }) => uuid)).toEqual(["one", "two"]);
  expect(mockFetch).toHaveBeenCalledTimes(2);
  for (const [url] of mockFetch.mock.calls) {
    expect(
      new URL(url as string, "https://openmrs.test").searchParams.has("form"),
    ).toBe(false);
  }
});
...
test.each([
  ...
  const { result } = renderHook(() =>
    useCanonicalVisitNoteEncounter(patient, visit, encounterType, form),
  );
  expect(result.current.status).toBe(status);
});

test("derives a stable UUID from the complete canonical identity", () => {
  const first = getCanonicalVisitNoteEncounterUuid(
    patient,
    visit,
    encounterType,
    form,
  ...
  expect(
    getCanonicalVisitNoteEncounterUuid(patient, visit, encounterType, form),
  ).toBe(first);
  expect(
    getCanonicalVisitNoteEncounterUuid(
      patient,
...
test("preflight exhausts the supported search and blocks an exact existing summary", async () => {
  ...
  await expect(
    assertCanonicalVisitNoteCanBeCreated(patient, visit, encounterType, form),
...
test("fails closed when an ambiguous create finds the same deterministic identity", async () => {
  const uuid = getCanonicalVisitNoteEncounterUuid(
    patient,
    visit,
    encounterType,
    form,
  ...
  mockFetch
    ...
    .mockResolvedValueOnce({ data: exactEncounter(uuid) } as never);

  await expect(
    saveCanonicalVisitNote(new AbortController(), payload),
  ).rejects.toBeInstanceOf(AmbiguousVisitNoteSaveError);
});

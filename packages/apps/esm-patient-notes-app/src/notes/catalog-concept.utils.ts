...
type CatalogConcept = Pick<Concept, 'conceptMappings' | 'display' | 'mappings' | 'names'>;
...
const prestacionalSourcePattern = /(?:^|\b)(?:sis|fua)(?:\b|$)|prestacion|codigo.*servicio/i;

const cie10PrefixPattern = /^\s*(?<code>[A-Z][0-9][A-Z0-9.]{1,5})\s*(?:[-–—:]\s*|\s+)(?<name>.+?)\s*$/i;
const cie10SuffixPattern = /^\s*(?<name>.+?)\s*\((?<code>[A-Z][0-9][A-Z0-9.]{1,5})\)\s*$/i;
const prestacionalPrefixPattern = /^\s*(?<code>\d{1,4}[A-Z]?)\s*(?:[-–—:]\s*|\s+)(?<name>.+?)\s*$/i;
const prestacionalSuffixPattern = /^\s*(?<name>.+?)\s*\((?<code>\d{1,4}[A-Z]?)\)\s*$/i;
...
function getMappingCode(mapping: ConceptReferenceMapping) {
  ...
  return (separatorIndex >= 0 ? display.slice(separatorIndex + 1) : display).trim();
}
 ...
 */
function getCatalogShortNameCode(concept: CatalogConcept, codePattern: RegExp): string | undefined {
...
function findMappedCode(concept: CatalogConcept, sourcePattern: RegExp, codePattern: RegExp) {
  ...
  const candidates = mappings.flatMap((mapping) => {
    ...
    return code && codePattern.test(code) ? [{ code, source: getMappingSource(mapping) }] : [];
  });
  const sourceMatch = candidates.find(({ source }) => sourcePattern.test(source));
...
function parseDisplayedCode(display: string, prefixPattern: RegExp, suffixPattern: RegExp): CatalogDisplayParts {
...
export function getCie10DisplayParts(concept: CatalogConcept): CatalogDisplayParts {
  const display = concept.display?.trim() ?? '';
  const displayedParts = parseDisplayedCode(display, cie10PrefixPattern, cie10SuffixPattern);
  const code =
 ...
 */
export function getCie10MappedCode(concept: CatalogConcept): string | undefined {
  const mappings = concept.conceptMappings ?? concept.mappings ?? [];
  ...
  const code = mappedCode?.conceptReferenceTerm?.code?.trim() || getCatalogShortNameCode(concept, cie10CodePattern);

  return code ? code.toLocaleUpperCase('es-PE') : undefined;
}

export function getPrestacionalDisplayParts(concept: CatalogConcept): CatalogDisplayParts {
  const display = concept.display?.trim() ?? '';
  const displayedParts = parseDisplayedCode(display, prestacionalPrefixPattern, prestacionalSuffixPattern);
  const code = findMappedCode(concept, prestacionalSourcePattern, prestacionalCodePattern) ?? displayedParts.code;
  return {
    code: code?.toLocaleUpperCase('es-PE'),
    name: displayedParts.name,
  };

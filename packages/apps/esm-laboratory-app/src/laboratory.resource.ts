...
export function useLabOrders(params: Partial<UseLabOrdersParams> = useLabOrdersDefaultParams) {
  ...
  const customRepresentation = `custom:(uuid,type,orderNumber,patient:(uuid,display,person:(uuid,display,age,birthdate,gender)${
  ...
  const extendedStartDate =
    dateRange && dateRange.at(0) ? dayjs(dateRange.at(0)).subtract(30, 'day').startOf('day').toDate() : null;

  url =
    dateRange && extendedStartDate
      ? `${url}&activatedOnOrAfterDate=${extendedStartDate.toISOString()}&activatedOnOrBeforeDate=${dayjs(
          dateRange.at(1),
        )
          .endOf('day')
          .toDate()
  ...
  const filteredOrders = data?.data?.results?.filter((order) => {
    if (
      newOrdersOnly &&
    ...
    const isInRange = (d: dayjs.Dayjs) => {
      return (d.isAfter(filterStart) || d.isSame(filterStart)) && (d.isBefore(filterEnd) || d.isSame(filterEnd));
    };
